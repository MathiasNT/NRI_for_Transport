...
def create_dataloaders_road(train_data, val_data, test_data, batch_size, normalize):
    ...
    train_index = torch.stack(
        [torch.arange(i, i + 24) for i in torch.arange(train_weather.shape[0])], 0
    )
    val_index = torch.stack(
        [torch.arange(i, i + 24) for i in torch.arange(val_weather.shape[0])], 0
    )
    test_index = torch.stack(
        [torch.arange(i, i + 24) for i in torch.arange(test_weather.shape[0])], 0
    )
