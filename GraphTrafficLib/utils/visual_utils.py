...
def plot_adj_on_map(adj_matrix, map_shp):
    # plot base map
    m = folium.Map(location=[40.8,-73.8], # center of the folium map
                   tiles='cartodbpositron', # type of map
                   zoom_start=12) # initial zoom

    # plot chorpleth over the base map
    folium.Choropleth(map_shp,                                # geo data
                      data=map_shp,                           # data
    ...
    for i, row in map_shp.iterrows():
        folium.CircleMarker(location=[row.geometry.centroid.y, row.geometry.centroid.x], popup=f"{row.LocationID}{row.zone}", radius=1).add_to(m)

    for idx, row in enumerate(adj_matrix):
        neighbour_idxs = torch.nonzero(row)
        zone_centroid = [map_shp.iloc[idx].geometry.centroid.y, map_shp.iloc[idx].geometry.centroid.x] 
        for neighbour_idx in neighbour_idxs:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                neighbour_centroid = [map_shp.iloc[neighbour_idx].geometry.centroid.y, map_shp.iloc[neighbour_idx].geometry.centroid.x]
...
def plot_directed_adj_on_map(adj_matrix, map_shp):
    ...
    for idx, row in enumerate(adj_matrix):
        ...
        for sender_idx in sender_idxs:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                sender_centroid = [map_shp.iloc[sender_idx].geometry.centroid.y, map_shp.iloc[sender_idx].geometry.centroid.x]
                line = folium.PolyLine([zone_centroid,  sender_centroid])
                line.add_to(m)
                PolyLineTextPath(line, " < ", repeat=True, center=True, offset=0, attributes={"font-size": "12"}).add_to(m)
