...
class DynamicGRUDecoder_multistep(nn.Module):
    """summary"""

    def __init__(self, n_hid, f_in, msg_hid, gru_hid, edge_types, skip_first):
        super().__init__()
    ...
    def do_single_step_forward(self, inputs, rel_rec, rel_send, rel_types, hidden, step):
    ...
    def forward(
        ...
        for step in range(0, inputs.shape[1] - 1):
            ...
            pred, hidden = self.do_single_step_forward(
                ins, rel_rec, rel_send, rel_types, hidden, step
            )
            pred_all.append(pred)
