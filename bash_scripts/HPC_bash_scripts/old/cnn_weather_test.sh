#!/bin/sh
#BSUB -J cnn_weather_test
#BSUB -q gpuv100
#BSUB -gpu "num=1:mode=exclusive_process"
#BSUB -n 4
#BSUB -W 24:00
#BSUB -R "span[hosts=1]"
#BSUB -R "rusage[mem=40GB]"
#BSUB -R "select[gpu32gb]"
#BSUB -N
#BSUB -o ../logs/%J_Output_cnn_weather_test.out
#BSUB -e ../logs/%J_Error_cnn_weather_test.err

EXP_NAME=cnn_weather_test
mkdir "../models/${EXP_NAME}"


EPOCHS=300
WEIGHT_DECAY=0.01
CUDA_DEVICE=0
BATCH_SIZE=100
BURN_IN_STEPS=30
SPLIT_LEN=40
EDGE_RATE=0.1
ENCODER_LR_RATE=0.1
N_EDGE_TYPES=2


PICKUP_DATA_PATH=full_manhattan/full_year_full_manhattan_2d.npy
WEATHER_DATA_PATH=LGA_weather_full_2019.csv


ENC_N_HID=64
python3 NRI_OD_train.py  --experiment_name ${EXP_NAME}/cnn_weather_test \
                        --epochs ${EPOCHS} \
                        --encoder_type cnn \
                        --loss_type nll \
                        --cuda_device ${CUDA_DEVICE} \
                        --pickup_data_name  ${PICKUP_DATA_PATH}\
                        --weather_data_name ${WEATHER_DATA_PATH} \
                        --batch_size ${BATCH_SIZE} \
                        --burn_in_steps ${BURN_IN_STEPS} \
                        --split_len ${SPLIT_LEN} \
                        --edge_rate ${EDGE_RATE} \
                        --encoder_lr_frac ${ENCODER_LR_RATE} \
			--enc_n_hid ${ENC_N_HID} \
			--gumbel_hard \
			--init_weights \
			--weight_decay ${WEIGHT_DECAY} \
			--use_weather
